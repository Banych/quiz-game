---
agent: 'agent'
description: 'Plan and execute complex features as a senior full-stack developer: produce a layered implementation plan, then execute it with test gates at each DDD layer.'
...
2. **For each task**:
   ...
   - Use Context7 to look up library APIs when uncertain (`mcp_context7_resolve-library-id` then `mcp_context7_search-docs`)
   - Implement changes
...
- Terminal for yarn commands, git operations
- Prisma CLI for migrations
- Supabase MCP for database inspection (`mcp_supabase_*`)
- Playwright MCP for E2E testing (`mcp_microsoft_pla_*`)
- Context7 MCP for live library documentation (`mcp_context7_*`)
