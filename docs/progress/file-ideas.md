...
| src/infrastructure/repositories/prisma-*.repository.ts      | Implement Quiz/Question/Answer Prisma repos + shared mappers/tests.           | TODO   |
| src/infrastructure/database/prisma/schema.prisma            | Add migrations, snake_case table mapping, and seed helpers.                   | TODO   |
| package.json                                                | Add `prisma:generate` / `prisma:migrate` scripts and reference in docs.       | TODO   |
| docs/01-setup-project.md                                    | Document DATABASE_URL/Supabase shadow DB setup + Prisma workflow.             | TODO   |
| src/app/api/**                                              | Create REST endpoints (join, start, submit answer) calling application layer. | TODO   |
